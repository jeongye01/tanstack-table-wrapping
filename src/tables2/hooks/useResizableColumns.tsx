...
export function useResizableColumns<TColumn>({
  initColumns,
}: UseResizableColumnsOptions<TColumn>) {
    ...
    const observer = new ResizeObserver((entries) => {
      for (const entry of entries) {
        if (entry.contentRect) {
          ...
          const staticWidth = columns.reduce(
            (acc, col) => acc + (col.size || 0),
            0
          );
          const defaultColumnWidth =
            dynamicColumns > 0
              ? Math.trunc((totalWidth - staticWidth) / dynamicColumns)
              : 0;
          const lastColumnWidth =
            totalWidth -
            staticWidth -
            defaultColumnWidth * (dynamicColumns - 1);
